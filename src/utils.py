...
def process_input_files(workspace, input):
    ...
    for file in files_temp:
        abs_path = path.join(workspace, file)

        if not file.endswith(".java"):
            print("[Warning] File '" + file + "' is not a Java file. Ignoring...")

        elif path.isfile(abs_path):
            files.append(abs_path)

        else:
            print("[Warning] File '" + file + "' not found in the repository. Ignoring...")
...
def process_input_paths(workspace, input):
    ...
    for p in paths_temp:
        abs_path = path.join(workspace, p)

        if path.isdir(abs_path):
            paths.append(abs_path)
        else:
            print("[Warning] Path '" + p + "' not found in the repository. Ignoring...")
